...
![인덱스](./_img/elasticsearch/elasticsearch_index.png)
...
데이터가 한번 색인 후 변경 불간으, 샤드에 기록된 데이터는 물리적으로 디스크에 저장된것 따라서 데이터가 보호된다.

### 노드

Elasticsearch 클러스터를 구성하는 개별 서버 인스턴스이다. 각 노드는 고유한 이름과 식별정보(ip, 포트 등)를 가지고 클러스터를 구성한다.

#### 마스터 노드

Elasticsearch 클러스터의 중심이 되는 노드, 클러스터 내 인덱스 생성 및 삭제, 노드 관리, 샤드 분배 등 중요한 결정을 내린다.

#### 데이터 노드

- **프라이머리 샤드** : 데이터 원본 관리, 색인 작업, 클러스터 핵심 데이터 저장, 인덱스 생성시 한번만 설정 가능하다.

- **레플리카 샤드** : 데이터 복사본 관리, 읽기 요청 부하 분산, 데이터 손상 방지, 프라이머리 샤드 장애시 레플리카 샤드가 승격(fail over)되어 프라이머리 샤드가 된다. 유연한 확장성
