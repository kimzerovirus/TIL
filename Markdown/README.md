...
<span>링크 만들기를 활용해 목차를 만들 수가 있어요 이 때 지켜줄 몇가지 규칙이 있어요</span>
<span>우선 링크의 목적지가 될 위치는 `#`으로 작성된 제목글이에요</span>
<span>`[목차 제목](링크)`링크를 작성할때는 맨 앞에 `#`을 써주고 이모지는 작성하지 않고 대소문자 구별없이 *소문자* 로 작성, 띄어쓰기는 `-`로 작성해주면 되요</span>
